...
/////////////////////////  
/*
  handleRemoveDeselect = (deselectedRemoveOptions) => {
    const selectedRemoveOptions = this.state.selectedRemoveOptions.slice();
    deselectedRemoveOptions.forEach(option => {
      selectedRemoveOptions.splice(selectedRemoveOptions.indexOf(option), 1);
    })
    this.setState({selectedRemoveOptions});
  }
  
  handleRemoveSelect = (selectedRemoveOptions) => {
    selectedRemoveOptions.sort((a, b) => a.id - b.id);
    this.setState({selectedRemoveOptions});
  }  
  
  handleNoFilterDeselect = (index) => {
    const selectedNoFilterOptions = this.state.selectedNoFilterOptions.slice();
    selectedNoFilterOptions.splice(index, 1);
    this.setState({selectedNoFilterOptions});
  }
  
  handleClearNoFilterSelection = (e) => {
    this.setState({selectedNoFilterOptions: []});
  }
  
  handleNoFilterSelectionChange = (selectedNoFilterOptions) => {
    selectedNoFilterOptions.sort((a, b) => a.id - b.id);
    this.setState({selectedNoFilterOptions});
  }
  ...
  handleSelectEquipment = (e, b) => {
	  const arr = [
		...
		[{id: 3, name: "2-1"},{id: 4, name: "2-2"}],
		[{id: 5, name: "3-1"},{id: 6, name: "3-2"}],
		[{id: 7, name: "4-1"},{id: 8, name: "4-2"}]
	  ];
